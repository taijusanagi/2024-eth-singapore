...
export class ProofGenerator {
  ...
  static async prepareState({
    ...
    const [
      defaultStartBlockSignup,
      defaultStartBlockPoll,
      { messageTreeDepth },
      stateRoot,
      numSignups,
    ] = await Promise.all([
      maciContract
        ...
        .then((events: any) => events[0]?.blockNumber ?? 0),
      maciContract
    ...
    const defaultStartBlock = Math.min(
      defaultStartBlockPoll,
      defaultStartBlockSignup
    );
    ...
    const defaultEndBlock = await Promise.all([
      pollContract
        .queryFilter(
          pollContract.filters.MergeMessageAq(messageRoot),
          fromBlock
        )
        .then((events: any) => events[events.length - 1]?.blockNumber),
      pollContract
        .queryFilter(
          pollContract.filters.MergeMaciState(stateRoot, numSignups),
          fromBlock
        )
        .then((events: any) => events[events.length - 1]?.blockNumber),
   ...
   */
  async generateMpProofs(options?: any): Promise<any[]> {
    ...
    try {
      ...
      while (this.poll.hasUnprocessedMessages()) {
        ...
        console.log(
          `Progress: ${this.poll.numBatchesProcessed} / ${totalMessageBatches}`
        );
      }
  ...
  async generateTallyProofs(
    network: any,
    options?: any
  ): Promise<{ proofs: any[]; tallyData: any }> {
    ...
    try {
      ...
      while (this.poll.hasUntalliedBallots()) {
        ...
        console.log(
          `Progress: ${this.poll.numBatchesTallied} / ${totalTallyBatches}`
        );
      }
      ...
      if (this.useQuadraticVoting) {
        ...
        newPerVOSpentVoiceCreditsCommitment = genTreeCommitment(
          ...
          BigInt(
            asHex(
              tallyCircuitInputs!
                .newPerVOSpentVoiceCreditsRootSalt as BigNumberish
            )
          ),
          this.poll.treeDepths.voteOptionTreeDepth
        );
