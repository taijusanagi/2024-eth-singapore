...
export const genMaciStateFromContract = async (
  ...
  const maciContract = new ethers.Contract(address, MACIAbi, provider);
  ...
  for (let i = fromBlock; i <= lastBlock; i += blocksPerRequest + 1) {
    ...
    for (let j = 0; j < deployPollLogs.length; j += 1) {
      const event = deployPollLogs[j] as any;
      assert(!!event);
  ...
  assert(
    coordinatorKeypair.pubKey.hash().toString() ===
      coordinatorPubKeyHashOnChain.toString()
  );
