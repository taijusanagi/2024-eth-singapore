...
export class TreeMerger {
   ...
   */
  private messageAccQueueContract: any;
  ...
  async checkPollDuration(): Promise<void> {
    ...
    const [deployTime, duration] =
      await this.pollContract.getDeployTimeAndDuration();
  ...
  async mergeMessageSubtrees(queueOps: number): Promise<void> {
    ...
    while (!subTreesMerged) {
      ...
      if (subTreesMerged) {
        ...
        // eslint-disable-next-line no-await-in-loop
        await this.messageAccQueueContract
          .getSrIndices()
          .then((indices: any) => {
            console.log(
              `Merging message subroots ${indices[0] + BigInt(1)} / ${
                indices[1] + BigInt(1)
              }`
            );
          });
  ...
  async mergeMessages(): Promise<void> {
    ...
    const messageTreeDepth = await this.pollContract
      .treeDepths()
      .then((depths: any) => Number(depths[2]));

    // check if the main root was not already computed
    const mainRoot = await this.messageAccQueueContract.getMainRoot(
