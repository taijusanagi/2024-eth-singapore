...
import { asHex, formatProofForVerifierContract } from "./utils";
...
export class Prover {
   ...
   * @param proofs - proofs
   */
  async proveMessageProcessing(proofs: any[]): Promise<void> {
    ...
    for (let i = numberBatchesProcessed; i < totalMessageBatches; i += 1) {
      ...
      if (numberBatchesProcessed === 0) {
        ...
        currentMessageBatchIndex =
          (totalMessageBatches - numberBatchesProcessed) * messageBatchSize;
      }
      ...
      // validation
      this.validatePollDuration(
        circuitInputs.pollEndTimestamp as BigNumberish,
        pollEndTimestampOnChain
      );
      this.validateMessageRoot(
        circuitInputs.msgRoot as BigNumberish,
        messageRootOnChain
      );
      ...
      if (numberBatchesProcessed === 0) {
        currentSbCommitmentOnChain = BigInt(
          await this.pollContract.currentSbCommitment()
        );
      } else {
        currentSbCommitmentOnChain = BigInt(
          await this.mpContract.sbCommitment()
        );
      }
      ...
      if (
        circuitInputs.coordinatorPublicKeyHash.toString() !==
        coordinatorPubKeyHash.toString()
      ) {
      ...
      try {
        ...
        numberBatchesProcessed = await this.mpContract
          .numBatchesProcessed()
          .then(Number);
   ...
   */
  async proveTally(proofs: any[]): Promise<void> {
    const [
      treeDepths,
      numSignUpsAndMessages,
      tallyBatchNumber,
    ...
    const tallyBatchSize =
      STATE_TREE_ARITY ** Number(treeDepths.intStateTreeDepth);
    ...
    for (let i = tallyBatchNum; i < totalTallyBatches; i += 1) {
      ...
      const currentTallyCommitmentOnChain =
        await this.tallyContract.tallyCommitment();
      this.validateCommitment(
        circuitInputs.currentTallyCommitment as BigNumberish,
        currentTallyCommitmentOnChain
      );

      const currentSbCommitmentOnChain = await this.mpContract.sbCommitment();
      console.log(currentSbCommitmentOnChain, circuitInputs);
      this.validateCommitment(
        circuitInputs.sbCommitment as BigNumberish,
        currentSbCommitmentOnChain
      );

      const publicInputsOnChain =
        await this.tallyContract.getPublicCircuitInputs(
          batchStartIndex,
      ...
      const vk = new VerifyingKey(
        ...
        onChainTallyVk.ic.map(([x, y]: any) => new G1Point(x, y))
      );
  ...
  private validatePollDuration(
    pollEndTimestamp: BigNumberish,
    pollEndTimestampOnChain: BigNumberish
  ) {
  ...
  private validateMessageRoot(
    messageRoot: BigNumberish,
    messageRootOnChain: BigNumberish
  ) {
  ...
  private validateCommitment(
    currentSbCommitment: BigNumberish,
    currentSbCommitmentOnChain: BigNumberish
  ) {
    if (
      currentSbCommitmentOnChain.toString() !== currentSbCommitment.toString()
    ) {
  ...
  private validatePublicInput(
    ...
    publicInputsOnChain: BigNumberish[]
  ) {
    if (
      !publicInputsOnChain.every(
        (value, index) => value.toString() === publicInputs[index].toString()
      )
    ) {
